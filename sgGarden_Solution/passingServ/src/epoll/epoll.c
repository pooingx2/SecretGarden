...
void userpool_add(int cli_fd, char *cli_ip, Peer *peer)
{
	...
	memset(&peer[i].userName, 0x00, 30);
	strcpy(&peer[i].ip[0],cli_ip);	// mac align
	printf("add User to UserTable / sock : %d / ip : %s \n", cli_fd, cli_ip);
...
void
userpool_add_name(int sock, Peer *peer, char nameBuf[])
{
	register int i;
	...
	for( i = 0 ; i < MAX_CLIENT ; i++ )
	{
		if(peer[i].socket == sock) 
			break;
	}


	/* Align Client [ip, id, socket, mac, acceable_directory] */
	strcpy(&peer[i].userName, nameBuf);	// mac align
	printf("add Name to UserTable / Name : %s \n", nameBuf);


}
int
get_id_from_data(char *org, char *dst, int len)
{
	int i;
	
	for(i=0; i<len; i++)
	{
		if(org[i] == "\t" || org[i] == "\0" || isblank(org[i]) )
		{
